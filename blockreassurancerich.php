...
include_once _PS_MODULE_DIR_.'blockreassurancerich/reassuranceClassRich.php';

class Blockreassurancerich extends Module implements WidgetInterface
{
    private $templateFile;

    public function __construct()
    {
        $this->name = 'blockreassurancerich';
        $this->author = 'rdy4ever';
        $this->version = '1.0.0';

        $this->bootstrap = true;
        parent::__construct();

        $this->displayName = $this->trans('Customer reassurance, with rich text', array(), 'Modules.Blockreassurancerich.Admin');
        $this->description = $this->trans('Same as original Block reassurnace by PrestaShop, but with rich text, allowing the formatting of reassurence items, with links and stuff. Attention: you should disable or uninstall the orignal blockreassurance module as this module replaces it.', array(), 'Modules.Blockreassurancerich.Admin');

        $this->ps_versions_compliancy = array('min' => '1.7.2.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:blockreassurancerich/views/templates/hook/blockreassurancerich.tpl';
    }
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'reassurancerich` (
        ...
        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'reassurancerich_lang` (
                `id_reassurance` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    {
        return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'reassurancerich`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'reassurancerich_lang`');
    }
    ...
    public function addToDB()
    {
        if (isset($_POST['nbblocks'])) {
            for ($i = 1; $i <= (int)$_POST['nbblocks']; $i++) {
                ...
                Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'reassurancerich` (`filename`,`text`)
    ...
    public function removeFromDB()
    {
        ...
        return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'reassurancerich`');
    }
    ...
    public function hookActionUpdateLangAfter($params)
    {
        if (!empty($params['lang']) && $params['lang'] instanceOf Language) {
            ...
            Language::updateMultilangFromClass(_DB_PREFIX_ . 'reassurancerich_lang', 'ReassuranceLang', $params['lang']);
        }
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('saveblockreassurancerich')) {
            if ($id_reassurance = Tools::getValue('id_reassurance')) {
                $reassurance = new reassuranceClassRich((int)$id_reassurance);
            } else {
                $reassurance = new reassuranceClassRich();
            }
        ...
        if (Tools::isSubmit('updateblockreassurancerich') || Tools::isSubmit('addblockreassurancerich')) {
        ...
        } elseif (Tools::isSubmit('deleteblockreassurancerich')) {
            $reassurance = new reassuranceClassRich((int)$id_reassurance);
        ...
        if (isset($_POST['submitModule'])) {
            ...
            if ($this->removeFromDB() && $this->addToDB()) {
                $this->_clearCache('blockreassurancerich.tpl');
                $output = '<div class="conf confirm">'.$this->trans('The block configuration has been updated.', array(), 'Modules.Blockreassurancerich.Admin').'</div>';
            } else {
    ...
    protected function getListContent($id_lang)
    {
        return  Db::getInstance()->executeS('
            ...
            FROM `'._DB_PREFIX_.'reassurancerich` r
            LEFT JOIN `'._DB_PREFIX_.'reassurancerich_lang` rl ON (r.`id_reassurance` = rl.`id_reassurance`)
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = array(
            'legend' => array(
                'title' => $this->trans('New reassurance block', array(), 'Modules.Blockreassurancerich.Admin'),
            ),
            'input' => array(
                array(
                    'type' => 'file',
        ...
        $helper->module = $this;
        $helper->name_controller = 'blockreassurancerich';
        ...
        $helper->submit_action = 'saveblockreassurancerich';
    ...
    protected function initList()
    {
        ...
        if (Shop::isFeatureActive()) {
            $this->fields_list['id_shop'] = array(
                'title' => $this->trans('ID Shop', array(), 'Modules.Blockreassurancerich.Admin'),
                'align' => 'center',
                'width' => 25,
                'type' => 'int'
            );
    ...
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('blockreassurancerich'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('blockreassurancerich'));
    }
    ...
    public function installFixtures()
    {
        $return = true;
        $tab_texts = array(
            array('text' => $this->trans('Security policy (edit with Customer reassurance Rich module)', array(), 'Modules.Blockreassurancerich.Shop'), 'file_name' => 'security.png'),
            array('text' => $this->trans('Delivery policy (edit with Customer reassurance Rich module)', array(), 'Modules.Blockreassurancerich.Shop'), 'file_name' => 'delivery.png'),
            array('text' => $this->trans('Return policy (edit with Customer reassurance Rich module)', array(), 'Modules.Blockreassurancerich.Shop'), 'file_name' => 'return.png'),
        );
